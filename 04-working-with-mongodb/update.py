...
def add_field(filename, fields):
    ...
    with open(filename, "r") as f:
        ...
        for i in range(3):
            l = reader.next()
        for line in reader:
            label = line['rdf-schema#label'].split(' (')[0]
...
def update_db(data, db):
    for record in data:
        updated = db.arachnid.find_one({'label': record})
        updated['classification']['binomialAuthority'] = data[record]
        db.arachnid.save(updated)
